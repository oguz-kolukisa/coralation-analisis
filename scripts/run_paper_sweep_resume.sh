#!/usr/bin/env bash
# Resume sweep starting from Waterbirds (Colored MNIST already complete).
# Cells 2/3/4: Waterbirds → NICO++ → ImageNet-100. Sequential.
set -euo pipefail
...
echo "[$(date +%H:%M:%S)] === Run 2/4: Waterbirds (14 models, 2 classes, 300 samples/class) ==="
...
# Dataset path: /workspace/data/nicopp/NICOpp (extracted from HF NICOpp.zip)
# ---------------------------------------------------------------------------
NICO_CLASSES=(
...
echo "[$(date +%H:%M:%S)] === ALL 3 RESUME RUNS COMPLETE ==="
